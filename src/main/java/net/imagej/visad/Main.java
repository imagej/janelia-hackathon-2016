...
public final class Main {
	...
	public static void main(final String... args) throws Exception {
		...
		double[][] grid = { // 12 grid points
			...
			{-.10*h, -.20*h, -.15*h, -.25*h,   .50*h, .45*h, .40*h, .55*h,  1.05*h, 1.03*h, 1.10*w, 1.07*h }
		};
		final Gridded2DDoubleSet set = new Gridded2DDoubleSet(type, grid, gw, gh);
		final GriddedSetTransform visadTransform = new GriddedSetTransform(set);

		// scale the image to the grid bounds
		final Scale2D shrink = new Scale2D((double) gw / w, (double) gh / h);
		final RealRandomAccessible<UnsignedByteType> shrunkRRA = RealViews.transformReal(rra, shrink);

		// apply the spiffy VisAD transform
		final RealTransformRandomAccessible<UnsignedByteType, InverseRealTransform> transformed =
			RealViews.transform(shrunkRRA, visadTransform);

		// reapply the original bounds (we set up the grid for this to work OK)
		final IntervalView<UnsignedByteType> transformedInterval = Views.interval(transformed, t0z0c0);
		final Img imgView = ImgView.wrap(transformedInterval, null);
